...
GRANT ALL PRIVILEGES ON wpTest.* TO 'root'@'localhost' IDENTIFIED BY 'tinnitus';
FLUSH PRIVILEGES;
EXIT
STOP
}
...
configuration() {
    ...
    perl -pi -e "s/database_name_here/wpTest/g" wp-config.php
    perl -pi -e "s/username_here/root/g" wp-config.php
    perl -pi -e "s/password_here/wpTest/g" wp-config.php
}
# Creating Exception
firewall() {
