...
database() {
    ...
    sudo debconf-set-selections <<< "mysql-server mysql-server/root_password password tinnitus"
    sudo debconf-set-selections <<< "mysql-server mysql-server/root_password_again password tinnitus"
    sudo apt-get install mysql-server -y
    echo "Creating Database"
    cat << STOP > /var/www/html/laraveldb.sql
CREATE DATABASE myDatabase;
CREATE USER 'osadmin'@'localhost' IDENTIFIED BY 'logmein';
GRANT ALL PRIVILEGES ON myDatabase.* TO 'osadmin'@'localhost';
FLUSH PRIVILEGES;
STOP
    mysql -u root -ptinnitus < /var/www/html/laraveldb.sql
}
...
restart() {
    ...
    echo "Laravel Access: http://localhost:8080"
    exit
}
# Calling Functions
webserver
